...
program
  ...
  .option('--output <format>', 'output format: text|json|jsonl|csv|table', 'text')
  ...
  .option('-b, --by <group>', 'group stats by: volume|quadrant|ring|all', 'all')
    ...
    .action(async (opts: any) => {
      ...
      if (out === 'jsonl') {
        ...
        if (stats.byVolume) console.log(JSON.stringify({ by: 'volume', data: stats.byVolume }));
        if (stats.byQuadrant) console.log(JSON.stringify({ by: 'quadrant', data: stats.byQuadrant }));
        if (stats.byRing) console.log(JSON.stringify({ by: 'ring', data: stats.byRing }));
        return;
      }
      ...
      if (out === 'csv') {
        ...
        if (opts.by === 'volume' || opts.by === 'all') printCSV(stats.byVolume, 'volume');
