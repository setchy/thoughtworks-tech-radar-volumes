...
export async function filterData(opts: FilterOpts) {
  ...
  const filtered = data.filter((entry) => {
    if (opts.volume && !String(entry.volume).toLowerCase().includes(opts.volume.toLowerCase())) return false;
    if (opts.quadrant && entry.quadrant.toLowerCase() !== opts.quadrant.toLowerCase()) return false;
    if (opts.ring && entry.ring.toLowerCase() !== opts.ring.toLowerCase()) return false;
    if (opts.status && String(entry.status).toLowerCase() !== opts.status.toLowerCase()) return false;
    ...
    if (opts.movement) {
      ...
      if (opts.movement === 'none' && (entry.hasMovedIn || entry.hasMovedOut)) return false;
    }
